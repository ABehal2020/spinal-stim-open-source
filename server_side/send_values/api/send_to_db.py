from ..models import SpinalStimData
from scipy.io import savemat
import matlab.engine
import base64
import os
 
def get_base64_encoded_image(image_path):
    with open(image_path, "rb") as img_file:
        return base64.b64encode(img_file.read()).decode('utf-8')

def insert_db(requestDict):
        sourceJobID = SpinalStimData.objects.filter(contactNum=requestDict["contactNum"], currentVal=requestDict["currentVal"], bodySide=requestDict["bodySide"], contactSymmetry=requestDict["contactSymmetry"], signalingProcMethod=requestDict["signalingProcMethod"], normalizationMethod=requestDict["normalizationMethod"], colormapOption=requestDict["colormapOption"], original=True).values("jobID")
        
        # sourceJobID = "88911b5b-9611-4387-81e3-60225211257d"
        
        print("source job ID")
        print(sourceJobID)

        if (not sourceJobID):
            # print(os.getcwd())
            # current working directory: /Users/adityabehal/Desktop/spinal-stim/server_side
            os.chdir("./send_values/api/matlab")
            # savemat("/Users/adityabehal/Desktop/spinal-stim/server_side/send_values/api/matlab/user_input.mat", requestDict, appendmat=False, format='5', long_field_names=True, do_compression=False, oned_as='row')
            savemat("user_input.mat", requestDict, appendmat=False, format='5', long_field_names=True, do_compression=False, oned_as='row')
            eng = matlab.engine.start_matlab()
            eng.addpath(r'.',nargout=0)
            # eng.addpath(r'/Users/adityabehal/Desktop/spinal-stim/server_side/send_values/api/matlab',nargout=0)
            # imageData = eng.heatMapScript(nargout=1)
            eng.heatMapScript(nargout=0)
            print("called matlab script")
            # imagePath = "/Users/adityabehal/Desktop/spinal-stim/server_side/send_values/api/results/" + requestDict["jobID"] + ".svg"
            imagePath = "../results/" + requestDict["jobID"] + ".svg"
            imageData = get_base64_encoded_image(imagePath)

            # imageData = ""
            
            imageDataBytes = str.encode(imageData)

            requestDict['imageData'] = imageDataBytes

            # f = open("imageData.txt", "wb")
            # f.write(requestDict['imageData'])
            # f.close()

            print("converted image to base64")
            requestDict["original"] = True
            requestDict["sourceJobID"] = "self"
            new_user_input = SpinalStimData.objects.create(**requestDict)
            new_user_input.save()
            print("saved NEW user input to postgres database")
        else:
            requestDict["imageData"] = None
            requestDict["original"] = False
            requestDict["sourceJobID"] = sourceJobID
            new_user_input = SpinalStimData.objects.create(**requestDict)
            new_user_input.save()
            print("saved input using CACHE to postgres database")