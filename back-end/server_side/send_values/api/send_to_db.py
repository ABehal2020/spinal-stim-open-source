from ..models import SpinalStimData
from scipy.io import savemat
import base64
from .heatmap import launchHeatMap
import os

def get_base64_encoded_image(image_path):
    with open(image_path, "rb") as img_file:
        return base64.b64encode(img_file.read()).decode("utf-8")

def insert_db(requestDict):
    sourceJobID = SpinalStimData.objects.filter(
        contactNum=requestDict["contactNum"],
        currentVal=requestDict["currentVal"],
        bodySide=requestDict["bodySide"],
        contactSymmetry=requestDict["contactSymmetry"],
        signalingProcMethod=requestDict["signalingProcMethod"],
        normalizationMethod=requestDict["normalizationMethod"],
        colormapOption=requestDict["colormapOption"],
        original=True,
    ).values("jobID")

    # sourceJobID = "88911b5b-9611-4387-81e3-60225211257d"

    print("source job ID")
    print(sourceJobID)

    if not sourceJobID:
        caseNumber = 4
        jobID = requestDict["jobID"]
        contactNum = requestDict["contactNum"]
        selectedCurrent = float(requestDict["currentVal"])
        bodySide = requestDict["bodySide"]
        contactSymmetry = requestDict["contactSymmetry"]
        out_method = requestDict["signalingProcMethod"]
        normalizationMethod = requestDict["normalizationMethod"]
        selectedColorMap = requestDict["colormapOption"]

        if (normalizationMethod == "dB"):
            normalizationMethodNum = 1
        elif (normalizationMethod == "percentage"):
            normalizationMethodNum = 2
        elif (normalizationMethod == "min"):
            normalizationMethodNum = 3
        elif (normalizationMethod == "group"):
            normalizationMethodNum = 4
        elif (normalizationMethod == "no"):
            normalizationMethodNum = 5

        launchHeatMap(caseNumber, contactNum, out_method, selectedCurrent, normalizationMethodNum, bodySide, selectedColorMap, jobID)
        print("called python heatmap script")
        print(os.getcwd())
        imagePath = "./send_values/api/results/" + requestDict["jobID"] + ".svg"
        imageData = get_base64_encoded_image(imagePath)

        # imageData = ""

        imageDataBytes = str.encode(imageData)

        requestDict["imageData"] = imageDataBytes

        print("converted image to base64")
        requestDict["original"] = True
        requestDict["sourceJobID"] = "self"
        new_user_input = SpinalStimData.objects.create(**requestDict)
        new_user_input.save()
        print("saved NEW user input to postgres database")
    else:
        requestDict["imageData"] = None
        requestDict["original"] = False
        requestDict["sourceJobID"] = sourceJobID
        new_user_input = SpinalStimData.objects.create(**requestDict)
        new_user_input.save()
        print("saved input using CACHE to postgres database")

